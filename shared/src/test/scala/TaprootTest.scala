...
object TaprootTest extends TestSuite {
  ...
  val tests = Tests {
    test("check taproot signatures") {
      ...
      assert(
        Script.pay2tr(outputKey) ==
          Script.parse(tx.txOut(0).publicKeyScript)
      )
      ...
      assert(Crypto.verifySignatureSchnorr(ourSig, hash, outputKey))

      // generate another sig with all zero random data, and check that it is valid too
      val ourSig1 = Crypto.signSchnorr(
        hash,
        priv,
        Some(
      ...
      assert(Crypto.verifySignatureSchnorr(ourSig1, hash, outputKey))
    }
    ...
    test("send to and spend from taproot addresses") {
      ...
      // we want to spend
      val outputScript = addressToPublicKeyScript(
      ...
      assert(ByteVector64(sig2) == sig)
      val sigHashType2: Int = if (sig2.size == 65) sig2(64).toInt else 0
      assert(sigHashType2 == sigHashType)
      val hash2 = Transaction.hashForSigningSchnorr(
        tx2,
        0,
        List(tx.txOut(1)),
        sigHashType2,
    ...
    test("use OP_CHECKSIGADD to implement multisig") {
      ...
      val script = List(
        OP_PUSHDATA(privs(0).publicKey.xonly),
        OP_CHECKSIG,
        OP_PUSHDATA(privs(1).publicKey.xonly),
        OP_CHECKSIGADD,
        OP_PUSHDATA(privs(2).publicKey.xonly),
        OP_CHECKSIGADD,
        OP_2,
      ...
      // we choose a pubkey that does not have a corresponding private key:
      ...
      val tx = tmp.updateWitness(
        0,
        ScriptWitness(
          List(sigs(0), sigs(0), sigs(0), Script.write(script), controlBlock)
        )
      )
      assert(
      ...
      val tx3 = tmp.updateWitness(
        0,
        ScriptWitness(
          List(sigs(2), sigs(1), sigs(0), Script.write(script), controlBlock)
        )
      )
      assert(
        Transaction.correctlySpends(
    ...
    test("create pay-to-script transactions") {
      ...
      val bip350Address =
        Bech32.encodeWitnessAddress("bcrt", 1.toByte, tweakedKey.value)
      ...
      assert(
        addressToPublicKeyScript(
          ...
          List(OP_1, OP_PUSHDATA(tweakedKey.value))
      )
      ...
      val tx1 = {
        ...
        val hash = Transaction.hashForSigningSchnorr(
          ...
          SigVersion.SIGVERSION_TAPSCRIPT,
          None,
          Some(ScriptTree.hash(leaves(0))),
          0xffffffffL
        )
        val sig = Crypto.signSchnorr(hash, privs(0), None)
        tmp.updateWitness(
          0,
          ScriptWitness(List(sig, Script.write(scripts(0)), controlBlock))
        )
      }
      assert(
        Transaction.correctlySpends(
      ...
      val tx2 = {
        ...
        val hash = Transaction.hashForSigningSchnorr(
          ...
          SigVersion.SIGVERSION_TAPSCRIPT,
          None,
          Some(ScriptTree.hash(leaves(1))),
          0xffffffffL
        )
        val sig = Crypto.signSchnorr(hash, privs(1), None)
        tmp.updateWitness(
          0,
          ScriptWitness(List(sig, Script.write(scripts(1)), controlBlock))
        )
      }
      assert(
        Transaction.correctlySpends(
      ...
      val tx3 = {
        ...
        val controlBlock = ByteVector(
          (Script.TAPROOT_LEAF_TAPSCRIPT + (if (parity) 1 else 0))
        ) ++
          internalPubkey.value ++
        ...
        val hash = Transaction.hashForSigningSchnorr(
          ...
          SigVersion.SIGVERSION_TAPSCRIPT,
          None,
          Some(ScriptTree.hash(leaves(2))),
          0xffffffffL
        )
        val sig = Crypto.signSchnorr(hash, privs(2), None)
        tmp.updateWitness(
          0,
          ScriptWitness(List(sig, Script.write(scripts(2)), controlBlock))
        )
      }
      assert(
        Transaction.correctlySpends(
