...
object Musig2Test extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("musig2 - key aggregation") {
      ...
      val pubkeys012 =
        pubkeys_bytes(0) :: pubkeys_bytes(1) :: pubkeys_bytes(2) :: Nil

      Musig2
        .keyAgg(pubkeys012.map(PublicKey(_)))
        .pointQ
        .xonly
        .value
        .toHex
        .toUpperCase ==> "90539EEDE565F5D054F32CC0C220126889ED1E5D193BAF15AEF344FE59D4610C"

      // same pubkeys as above, different order
      val pubkeys210 =
        pubkeys_bytes(2) :: pubkeys_bytes(1) :: pubkeys_bytes(0) :: Nil

      Musig2
        .keyAgg(pubkeys210.map(PublicKey(_)))
        .pointQ
        .xonly
        .value
        .toHex
      ...
      val pubkeys000 =
        pubkeys_bytes(0) :: pubkeys_bytes(0) :: pubkeys_bytes(0) :: Nil
      Musig2
        .keyAgg(pubkeys000.map(PublicKey(_)))
        .pointQ
        .xonly
        .value
        .toHex
      ...
      val pubkeys0011 = pubkeys_bytes(0) :: pubkeys_bytes(0) :: pubkeys_bytes(
        1
      ) :: pubkeys_bytes(1) :: Nil
      Musig2
        .keyAgg(pubkeys0011.map(PublicKey(_)))
        .pointQ
        .xonly
        .value
        .toHex
    ...
    test("musig2 - nonce generation 1") {
      ...
      val (secnonce, pubnonce) = Musig2.nonceGen(
        ...
        nextRand32 = ByteVector32.fromValidHex(
          "0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F0F"
        )
      )
      val expectedSecNonce = ByteVector.fromValidHex(
        "B114E502BEAA4E301DD08A50264172C84E41650E6CB726B410C0694D59EFFB6495B5CAF28D045B973D63E3C99A44B807BDE375FD6CB39E46DC4A511708D0E9D2024D4B6CD1361032CA9BD2AEB9D900AA4D45D9EAD80AC9423374C451A7254D0766"
      )
    ...
    test("musig2 - nonce generation 2") {
      ...
      val expectedSecNonce = ByteVector.fromValidHex(
        "227243DCB40EF2A13A981DB188FA433717B506BDFA14B1AE47D5DC027C9C3B9EF2370B2AD206E724243215137C86365699361126991E6FEC816845F837BDDAC3024D4B6CD1361032CA9BD2AEB9D900AA4D45D9EAD80AC9423374C451A7254D0766"
      )
      assert(secnonce == expectedSecNonce)
    }
    ...
    test("musig2 - nonce generation 3") {
      ...
      val expectedSecNonce = ByteVector.fromValidHex(
        "CD0F47FE471D6788FF3243F47345EA0A179AEF69476BE8348322EF39C2723318870C2065AFB52DEDF02BF4FDBF6D2F442E608692F50C2374C08FFFE57042A61C024D4B6CD1361032CA9BD2AEB9D900AA4D45D9EAD80AC9423374C451A7254D0766"
      )
      assert(secnonce == expectedSecNonce)
    }
    ...
    test("musig2 - nonce generation 4") {
      ...
      val expectedSecNonce = ByteVector.fromValidHex(
        "011F8BC60EF061DEEF4D72A0A87200D9994B3F0CD9867910085C38D5366E3E6B9FF03BC0124E56B24069E91EC3F162378983F194E8BD0ED89BE3059649EAE262024D4B6CD1361032CA9BD2AEB9D900AA4D45D9EAD80AC9423374C451A7254D0766"
      )
      assert(secnonce == expectedSecNonce)
    }
    ...
    test("musig2 - nonce generation 5") {
      ...
      val expectedSecNonce = ByteVector.fromValidHex(
        "890E83616A3BC4640AB9B6374F21C81FF89CDDDBAFAA7475AE2A102A92E3EDB29FD7E874E23342813A60D9646948242646B7951CA046B4B36D7D6078506D3C9402F9308A019258C31049344F85F89D5229B531C845836F99B08601F113BCE036F9"
      )
      assert(secnonce == expectedSecNonce)
    }
    ...
    test("musig2 - nonce aggregation") {
      ...
      assert(
        Musig2.nonceAgg(pnonces01) == ByteVector.fromValidHex(
    ...
    test("musig2 - partial signature aggregatation") {
      ...
      { // test 1
        val aggnonce = ByteVector.fromValidHex(
        ...
        assert(
          Musig2.partialSigAgg(sigs, ctx) == ByteVector64.fromValidHex(
      ...
      { // test 4 (now including some tweaks)
        ...
        val ctx = Musig2.SessionCtx(
          aggNonce = ByteVector.fromValidHex(
          ...
          numTweaks = 3,
          tweaks =
            (tweaks(0) :: tweaks(1) :: tweaks(2) :: Nil).map(ByteVector32(_)),
    ...
    test("musig2 - partial signature verification") {
      ...
      def innerTestAssertValid(
        ...
        /** # WARNING: An actual implementation should _not_ copy the secnonce.
          * Reusing the secnonce, as we do here for testing purposes, can leak
          * the secret key.
          */
      ...
      innerTestAssertSignFail(
        ...
        comment =
          "Aggregate nonce is invalid due to wrong tag, 0x04, in first half"
      )
      innerTestAssertSignFail(
        key_indices = List(1, 2, 0),
        aggnonce_index = 3,
      ...
      innerTestAssertSignFail(
        ...
        error_message = "invalid_contribution (aggnonce)",
        comment =
          "Aggregate nonce is invalid because the second half exceeds field size"
      )
      innerTestAssertSignFail(
        key_indices = List(0, 1, 2),
        aggnonce_index = 0,
