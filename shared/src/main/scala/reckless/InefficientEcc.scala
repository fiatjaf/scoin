...
/** The elliptic curve domain parameters over F_p associated with a Koblitz
  * curve E of the form y^2 = x^3 + a*x + b mod p
  */
trait Curve[A] {
  type F_p = BigInt
  ...
  object CurvePoint {
    def fromUnCompressed(uncompressed: ByteVector): CurvePoint = {
      ...
      CurvePoint(
        BigInt(uncompressed.drop(1).take(32).toHex, 16),
        BigInt(uncompressed.drop(33).toHex, 16)
      )
    }
  }
...
object Curve {
  ...
  def isPointOnCurve[A](curve: Curve[A])(pt: curve.CurvePoint): Boolean = {
    pt.y.pow(2).mod(curve._p) == (pt.x.pow(3).mod(curve._p) + (pt.x * curve._a)
      .mod(curve._p) + curve._b).mod(curve._p)
  }

  def pointNegate[A](curve: Curve[A])(pt: curve.CurvePoint): curve.CurvePoint =
    curve.CurvePoint(pt.x, curve._p - pt.y)
  ...
  def pointDouble[A](curve: Curve[A])(pt: curve.Point): curve.Point = pt match {
    ...
    case curve.CurvePoint(x, y) => {
      val three = BigInt(3)
      val two = BigInt(2)
      val L = (((three * x.pow(2)).mod(curve._p) + curve._a)
        .mod(curve._p) * (two * y).modInverse(curve._p)).mod(curve._p)
  ...
  def pointAdd[A](
      curve: Curve[A]
  )(lhs: curve.Point, rhs: curve.Point): curve.Point = (lhs, rhs) match {
    ...
    case (a: curve.CurvePoint, b: curve.CurvePoint) => {
      val L = ((b.y - a.y).mod(curve._p) * (b.x - a.x).modInverse(curve._p))
        .mod(curve._p)
  ...
  def multByScalar[A](
      curve: Curve[A]
  )(point: curve.Point, scalar: BigInt): curve.Point = {
...
trait Secp256k1 extends Curve[Secp256k1] {
  ...
  /** The elliptic curve domain parameters over F_p associated with a Koblitz
    * curve secp256k1 are specified by the sextuple T = (p,a,b,G,n,h) where the
    ...
    * 00000000 00000007 The base point G in compressed form is:
    *
    ...
    * FB10D4B8
    *
    * Finally the order n of G and the cofactor are: n = FFFFFFFF FFFFFFFF
    * FFFFFFFF FFFFFFFE BAAEDCE6 AF48A03B BFD25E8C D0364141 h = 01
    *
    * Helpful description of the difference between the prime modulus p, and _n,
    * the order of G:
    * https://crypto.stackexchange.com/questions/86328/secp256k1-prime-modulus-vs-order
    */
