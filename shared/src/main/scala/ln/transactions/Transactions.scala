package scoin.ln.transactions

import java.nio.ByteOrder
import scala.util.Try
import scodec.bits.ByteVector
import scoin._
import scoin.Crypto.{PrivateKey, PublicKey, ripemd160}
import scoin.Script._
import scoin.SigVersion._
import scoin.ln._
import scoin.ln.transactions.CommitmentOutput._
...
object Transactions {
  ...
  /** This commitment format may be unsafe where you're fundee, as it exposes
    * you to a fee inflating attack. Don't use this commitment format unless you
    * know what you're doing! See
    * https://lists.linuxfoundation.org/pipermail/lightning-dev/2020-September/002796.html
    * for details.
    */
  case object UnsafeLegacyAnchorOutputsCommitmentFormat
      extends AnchorOutputsCommitmentFormat

  /** This commitment format removes the fees from the pre-signed 2nd-stage htlc
    * transactions to fix the fee inflating attack against
  ...
  object InputInfo {
    def apply(outPoint: OutPoint, txOut: TxOut, redeemScript: Seq[ScriptElt]) =
      new InputInfo(outPoint, txOut, Script.write(redeemScript))
  }
  ...
  sealed trait ReplaceableTransactionWithInputInfo
  ...
  case class CommitTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo {
    override def desc: String = "commit-tx"
  }

  /** It's important to note that htlc transactions with the default commitment
    * format are not actually replaceable: only anchor outputs htlc transactions
    * are replaceable. We should have used different types for these different
    * kinds of htlc transactions, but we introduced that before implementing the
    * replacement strategy. Unfortunately, if we wanted to change that, we would
    * have to update the codecs and implement a migration of channel data, which
    * isn't trivial, so we chose to temporarily live with that inconsistency
    * (and have the transaction replacement logic abort when non-anchor outputs
    * htlc transactions are provided). Ideally, we'd like to implement a dynamic
    * commitment format upgrade mechanism and depreciate the pre-anchor outputs
    * format soon, which will get rid of this inconsistency. The next time we
    * introduce a new type of commitment, we should avoid repeating that mistake
    * and define separate types right from the start.
    */
  sealed trait HtlcTx extends ReplaceableTransactionWithInputInfo {
    def htlcId: Long
    override def sighash(
        txOwner: TxOwner,
        commitmentFormat: CommitmentFormat
    ): Int = commitmentFormat match {
  ...
  case class HtlcSuccessTx(
      input: InputInfo,
      tx: Transaction,
      paymentHash: ByteVector32,
      htlcId: Long,
      confirmBefore: BlockHeight
  ) extends HtlcTx { override def desc: String = "htlc-success" }
  case class HtlcTimeoutTx(
      input: InputInfo,
      tx: Transaction,
      htlcId: Long,
      confirmBefore: BlockHeight
  ) extends HtlcTx { override def desc: String = "htlc-timeout" }
  case class HtlcDelayedTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo {
  ...
  case class LegacyClaimHtlcSuccessTx(
      input: InputInfo,
      tx: Transaction,
      htlcId: Long,
      confirmBefore: BlockHeight
  ) extends ClaimHtlcTx { override def desc: String = "claim-htlc-success" }
  case class ClaimHtlcSuccessTx(
      input: InputInfo,
      tx: Transaction,
      paymentHash: ByteVector32,
      htlcId: Long,
  ...
  case class ClaimHtlcTimeoutTx(
      input: InputInfo,
      tx: Transaction,
      htlcId: Long,
      confirmBefore: BlockHeight
  ) extends ClaimHtlcTx { override def desc: String = "claim-htlc-timeout" }
  sealed trait ClaimAnchorOutputTx extends TransactionWithInputInfo
  case class ClaimLocalAnchorOutputTx(
      input: InputInfo,
      tx: Transaction,
      confirmBefore: BlockHeight
  ) extends ClaimAnchorOutputTx
      with ReplaceableTransactionWithInputInfo {
    override def desc: String = "local-anchor"
  }
  case class ClaimRemoteAnchorOutputTx(input: InputInfo, tx: Transaction)
      extends ClaimAnchorOutputTx {
    override def desc: String = "remote-anchor"
  }
  sealed trait ClaimRemoteCommitMainOutputTx extends TransactionWithInputInfo
  case class ClaimP2WPKHOutputTx(input: InputInfo, tx: Transaction)
      extends ClaimRemoteCommitMainOutputTx {
    override def desc: String = "remote-main"
  }
  case class ClaimRemoteDelayedOutputTx(input: InputInfo, tx: Transaction)
      extends ClaimRemoteCommitMainOutputTx {
    override def desc: String = "remote-main-delayed"
  }
  case class ClaimLocalDelayedOutputTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo {
    override def desc: String = "local-main-delayed"
  }
  case class MainPenaltyTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo {
    override def desc: String = "main-penalty"
  }
  case class HtlcPenaltyTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo {
    override def desc: String = "htlc-penalty"
  }
  case class ClaimHtlcDelayedOutputPenaltyTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo {
    override def desc: String = "htlc-delayed-penalty"
  }
  case class ClosingTx(
      input: InputInfo,
      tx: Transaction,
      toLocalOutput: Option[OutputInfo]
  ) extends TransactionWithInputInfo { override def desc: String = "closing" }

  sealed trait TxGenerationSkipped
  case object OutputNotFound extends TxGenerationSkipped {
    override def toString = "output not found (probably trimmed)"
  }
  case object AmountBelowDustLimit extends TxGenerationSkipped {
    override def toString = "amount is below dust limit"
  }
  // @formatter:on
  ...
  /** When *local* *current* [[CommitTx]] is published:
    ...
    *     after a delay
    *   - When using anchor outputs, [[ClaimLocalAnchorOutputTx]] spends
  ...
  def offeredHtlcTrimThreshold(
    ...
    commitmentFormat match {
      ...
      case _ =>
        dustLimit + weight2fee(feerate, commitmentFormat.htlcTimeoutWeight)
    }
  }
  ...
  def receivedHtlcTrimThreshold(
    ...
    commitmentFormat match {
      case ZeroFeeHtlcTxAnchorOutputsCommitmentFormat => dustLimit
      case _ =>
        dustLimit + weight2fee(feerate, commitmentFormat.htlcSuccessWeight)
    }
  }
  ...
  /** While on-chain amounts are generally computed in Satoshis (since this is
    * the smallest on-chain unit), it may be useful in some cases to calculate
    * it in MilliSatoshi to avoid rounding issues. If you are adding multiple
    * fees together for example, you should always add them in MilliSatoshi and
    * then round down to Satoshi.
    */
  def commitTxTotalCostMsat(
      dustLimit: Satoshi,
      spec: CommitmentSpec,
      commitmentFormat: CommitmentFormat
  ): MilliSatoshi = {
  ...
  def commitTxTotalCost(
      dustLimit: Satoshi,
      spec: CommitmentSpec,
      commitmentFormat: CommitmentFormat
  ): Satoshi =
    commitTxTotalCostMsat(dustLimit, spec, commitmentFormat).truncateToSatoshi
  ...
  def makeCommitTxOutputs(
    ...
    val (toLocalAmount: Satoshi, toRemoteAmount: Satoshi) =
      if (localIsInitiator) {
        (
          spec.toLocal.truncateToSatoshi - commitTxTotalCost(
            localDustLimit,
            spec,
            commitmentFormat
          ),
        ...
        (
          ...
          spec.toRemote.truncateToSatoshi - commitTxTotalCost(
            localDustLimit,
            spec,
            commitmentFormat
          )
  ...
  def makeHtlcTimeoutTx(
    ...
    if (amount < localDustLimit) {
      ...
      Right(
        HtlcTimeoutTx(input, tx, htlc.id, BlockHeight(htlc.cltvExpiry.toLong))
      )
    }
  }
    ...
    if (amount < localDustLimit) {
      ...
      Right(
        HtlcSuccessTx(
          ...
          htlc.paymentHash,
          htlc.id,
          BlockHeight(htlc.cltvExpiry.toLong)
        )
      )
    }
  }
  ...
  def makeClaimLocalDelayedOutputTx(
    ...
    makeLocalDelayedOutputTx(
      ...
      ClaimLocalDelayedOutputTx(input, tx)
    }
  }
  ...
  private def makeLocalDelayedOutputTx(
      ...
      localFinalScriptPubKey: ByteVector,
      feeratePerKw: FeeratePerKw
  ): Either[TxGenerationSkipped, (InputInfo, Transaction)] = {
    ...
    findPubKeyScriptIndex(parentTx, pubkeyScript) match {
      ...
      case Right(outputIndex) =>
        val input = InputInfo(
          OutPoint(parentTx, outputIndex),
          parentTx.txOut(outputIndex),
          write(redeemScript)
        )
        // unsigned transaction
        val tx = Transaction(
          version = 2,
          txIn =
            TxIn(input.outPoint, ByteVector.empty, toLocalDelay.toInt) :: Nil,
          txOut = TxOut(Satoshi(0), localFinalScriptPubKey) :: Nil,
          lockTime = 0
        )
        // compute weight with a dummy 73 bytes signature (the largest you can get)
  ...
  private def makeClaimAnchorOutputTx(
      commitTx: Transaction,
  ...
  def makeClaimLocalAnchorOutputTx(
      commitTx: Transaction,
      localFundingPubkey: PublicKey,
      confirmBefore: BlockHeight
  ): Either[TxGenerationSkipped, ClaimLocalAnchorOutputTx] = {
    makeClaimAnchorOutputTx(commitTx, localFundingPubkey).map {
      case (input, tx) => ClaimLocalAnchorOutputTx(input, tx, confirmBefore)
    }
  }

  def makeClaimRemoteAnchorOutputTx(
      commitTx: Transaction,
      remoteFundingPubkey: PublicKey
  ): Either[TxGenerationSkipped, ClaimRemoteAnchorOutputTx] = {
    makeClaimAnchorOutputTx(commitTx, remoteFundingPubkey).map {
      case (input, tx) => ClaimRemoteAnchorOutputTx(input, tx)
    }
  }
  ...
  def makeClaimHtlcDelayedOutputPenaltyTxs(
      ...
      localFinalScriptPubKey: ByteVector,
      feeratePerKw: FeeratePerKw
  ): Seq[Either[TxGenerationSkipped, ClaimHtlcDelayedOutputPenaltyTx]] = {
    val redeemScript = toLocalDelayed(
      localRevocationPubkey,
      toLocalDelay,
      localDelayedPaymentPubkey
      ...
      case Right(outputIndexes) =>
        outputIndexes.map(outputIndex => {
          ...
          if (amount < localDustLimit) {
            ...
            val tx1 =
              tx.copy(txOut = tx.txOut.head.copy(amount = amount) :: Nil)
  ...
  def makeClosingTx(
    ...
    val toLocalOutput = findPubKeyScriptIndex(tx, localScriptPubKey)
      .map(index => OutputInfo(index, toLocalAmount, localScriptPubKey))
      .toOption
  ...
  def findPubKeyScriptIndexes(
      tx: Transaction,
      pubkeyScript: ByteVector
  ): Either[TxGenerationSkipped, Seq[Int]] = {
  ...
  def addSigs(
      htlcDelayedTx: HtlcDelayedTx,
      localSig: ByteVector64
  ): HtlcDelayedTx = {
    val witness = witnessToLocalDelayedAfterDelay(
      localSig,
      htlcDelayedTx.input.redeemScript
    )
    htlcDelayedTx.copy(tx = htlcDelayedTx.tx.updateWitness(0, witness))
  }

  def addSigs(
      claimAnchorOutputTx: ClaimLocalAnchorOutputTx,
      localSig: ByteVector64
