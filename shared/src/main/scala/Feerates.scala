...
object FeeratePerKB {
  def apply(feeratePerByte: FeeratePerByte): FeeratePerKB = FeeratePerKB(
    feeratePerByte.feerate * 1000
  )
  def apply(feeratePerKw: FeeratePerKw): FeeratePerKB = FeeratePerKB(
    feeratePerKw.feerate * 4
  )
}
...
case class FeeratePerKw(feerate: Satoshi) extends Ordered[FeeratePerKw] {
  ...
  def +(other: FeeratePerKw): FeeratePerKw = FeeratePerKw(
...
object FeeratePerKw {
  ...
  /** Why 253 and not 250 since feerate-per-kw should be feerate-per-kvb / 4 and
    ...
    */
  val MinimumFeeratePerKw = FeeratePerKw(Satoshi(253))

  def apply(feeratePerKB: FeeratePerKB): FeeratePerKw =
    MinimumFeeratePerKw.max(FeeratePerKw(feeratePerKB.feerate / 4))
  def apply(feeratePerByte: FeeratePerByte): FeeratePerKw =
    FeeratePerKw(FeeratePerKB(feeratePerByte))
}
