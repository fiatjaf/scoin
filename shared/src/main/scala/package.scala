...
package object scoin {
  ...
  val SIGHASH_DEFAULT: Int =
    0 // !< Taproot only; implied when sighash byte is missing, and equivalent to SIGHASH_ALL
  val SIGHASH_ANYPREVOUT = 0x40
  val SIGHASH_ANYPREVOUTANYSCRIPT = 0xc0
  ...
  /** Given an address and chain code, find public key script (if exists). Will
    * try Base58 encoding first, then Bech32.
    *
    * @param chainHash
    * @param address
    * @return
      ...
      case Success((prefix, data)) =>
        prefix match {
          case Base58.Prefix.PubkeyAddressTestnet
            ...
            Script.pay2pkh(data)
          case Base58.Prefix.PubkeyAddress
              if (chainHash == Block.LivenetGenesisBlock.hash) =>
            Script.pay2pkh(data)
          case Base58.Prefix.ScriptAddressTestnet
              if (chainHash == Block.TestnetGenesisBlock.hash || chainHash == Block.RegtestGenesisBlock.hash || chainHash == Block.SignetGenesisBlock.hash) =>
            List(OP_HASH160, OP_PUSHDATA(data), OP_EQUAL)
          case Base58.Prefix.ScriptAddress
              if (chainHash == Block.LivenetGenesisBlock.hash) =>
            List(OP_HASH160, OP_PUSHDATA(data), OP_EQUAL)
          case _ =>
            throw new IllegalArgumentException(
        ...
        Try(Bech32.decodeWitnessAddress(address)) match {
          case Success((prefix, version, program)) => {
            witnessVersions.get(version) match {
              ...
              case Some(wv) if (program.size != 20 && program.size != 32) =>
                throw new IllegalArgumentException(
                  "hash length in bech32 address must be either 20 or 32 bytes"
                )
              case Some(wv)
                  if (prefix == "bc" && chainHash == Block.LivenetGenesisBlock.hash) =>
                List(wv, OP_PUSHDATA(program))
              case Some(wv)
                  if (prefix == "tb" && chainHash == Block.TestnetGenesisBlock.hash) =>
                List(wv, OP_PUSHDATA(program))
              case Some(wv)
                  if (prefix == "tb" && chainHash == Block.SignetGenesisBlock.hash) =>
                List(wv, OP_PUSHDATA(program))
              case Some(wv)
                  if (prefix == "bcrt" && chainHash == Block.RegtestGenesisBlock.hash) =>
                List(wv, OP_PUSHDATA(program))
              case _ =>
                throw new IllegalArgumentException(
          ...
          case Failure(exception) =>
            throw new IllegalArgumentException(
              s"$address is neither a valid Base58 address ($base58error) nor a valid Bech32 address"
            )
        }
    }
  }
